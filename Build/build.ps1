...
properties {
  ...
  $zipFileName = "JsonSchema$($versionInfo.Major)0r$($versionInfo.Release).zip"
  ...
  $builds = @(
    ...
    @{Framework = "netstandard2.1"; TestsFunction = "NetCliTests"; TestFramework = "net10.0"; Enabled=$true}
...
task Package -depends Build {
  foreach ($build in $script:enabledBuilds)
  {
    ...
    $sourcePath = "$sourceDir\Newtonsoft.Json.Schema\bin\Release\$finalDir"
    ...
    robocopy $sourcePath $workingDir\Package\Bin\$finalDir Newtonsoft.Json.Schema.dll Newtonsoft.Json.Schema.pdb Newtonsoft.Json.Schema.xml /NFL /NDL /NJS /NC /NS /NP /XO /XF *.CodeAnalysisLog.xml | Out-Default
  }
  ...
  if ($buildNuGet)
  {
    ...
    move -Path $sourceDir\Newtonsoft.Json.Schema\bin\Release\*.nupkg -Destination $workingDir\NuGet
    move -Path $sourceDir\Newtonsoft.Json.Schema\bin\Release\*.snupkg -Destination $workingDir\NuGet
  }
  ...
  if ($buildDocumentation)
  {
    ...
    $mainBuildFinalDir = if ($mainBuild.TestFramework -ne $null) { $mainBuild.TestFramework } else { $build.Framework }
...
function EnsureDotnetCli()
{
  ...
  exec { & $buildDir\Temp\dotnet-install.ps1 -Channel '8.0' -Runtime 'dotnet' | Out-Default }
}
...
{
  $projectPath = "$sourceDir\Newtonsoft.Json.Schema.Tests\Newtonsoft.Json.Schema.Tests.csproj"
...
function NUnitTests($build)
{
  ...
  $testRunDir = "$sourceDir\Newtonsoft.Json.Schema.Tests\bin\Release\$testDir"
