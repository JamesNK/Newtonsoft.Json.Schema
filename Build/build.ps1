properties { 
  $zipFileName = "JsonSchema30r5.zip"
  $majorVersion = "3.0"
  $majorWithReleaseVersion = "3.0.5"
...
task Package -depends Build {
  foreach ($build in $script:enabledBuilds)
  {
    ...
    $sourcePath = "$workingSourceDir\Newtonsoft.Json.Schema\bin\Release\$finalDir"
  ...
  if ($buildNuGet)
  {
    ...
    exec { & $script:msBuildPath "/t:pack" "/v:$msbuildVerbosity" "/p:IncludeSource=true" "/p:Configuration=Release" "/p:TargetFrameworks=`"$targetFrameworks`"" "$workingSourceDir\Newtonsoft.Json.Schema\Newtonsoft.Json.Schema.csproj" }
